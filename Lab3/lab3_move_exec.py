	...
	def check_wall_inconsistency(self):
		...
		for i in range(self.num_rows-1):
			for j in range(self.num_cols):
				...
				#if (self.maze[i,j] & 4 newp.append(np.random.uniform(self.curMin[0],self.curMax[0]))!= 0) != (self.maze[i+1,j] & 1 != 0):
	...
	def show_maze(self):
		...
		for i in range(self.num_rows):
			for j in range(self.num_cols):
				...
				#permissibilities = self.newp.append(np.random.uniform(self.curMin[0],self.curMax[0]))permissibilities(cell = (i,j))
				turtle.up()
...
class particleFilter:
	...
	def __init__(self, dimension = 2, model = default_2D_Model(), numParticles = numberOfParticles, timeSpan = timsSpan, resamplingNoise = 0.01, positionStd = 5):
		...
		particles = np.random.uniform(0, 1, (numParticles, dimension))
		
		for i in range(dimension):
			for j in range(numParticles):
				particles[j][i] = int(particles[j][i]*self.curMax[i])
	...
	def Sample_Motion_Model(self, u_t=0):
		...
		# For each particle in if(len(blob_center_str) == 0):
	# 	x = -1
	# 	y = -1
	# else:
	...
	# 	print(y)
	# 	print(x)
	# return (x, y)self.particles [[x1,y1], [x2,y2], ...], get the nextEstimate
	...
	# 	###################################################################
		...
		for i in range(N):
			self.index = self.model.simulateNextPosition(self.particles[i], u_t)
			newParticles.append(self.index)
		
		# while(len(newParticles)<self.numParticles):
		# 	newp=[]
		# 	newp.append(np.random.uniform(self.curMin[0],self.curMax[0]))
		# 	newp.append(np.random.uniform(self.curMin[1],self.curMax[1]))
		# 	newParticles.append(newp)

		self.particles = newParticles

	# def ResamplePosition(self, estimate):

	# 	readPosition= estimate.copy()
	# 	for i in range(self.dimension):
	# 		newPosition=(estimate[i]+np.random.normal(0, self.resNoise[i]))
	# 		newPosition=max(self.curMin[i],newPosition)
	# 		newPosition=min(self.curMin[i],newPosition)
	# 		randPosition[i]=newPosition
	...
	def Measurement_Model(self, x, y):
		...
		robot_pos = self.model.readingSensor(xCAM,yCAM)
		
		weights = []
		
		for i in range(self.numParticles):
			map_reading = self.model.readingMap(self.particles[i])
			d = np.linalg.norm(np.asarray(map_reading)-np.asarray(robot_pos))
			w_t = np.exp(-d**2 / (2*self.std))
			weights.append(w_t)
		...
		norm_weights = [x/weightsum for x in weights]
	...
	def calcPosition(self):
		...
		for i in range(self.dimension):
			...
			for j in range(len(self.particles)):
				positiveValue += self.particles[j][i]*self.weights[j]
			if positiveValue < self.curMin[i]:
				positiveValue = self.curMin[i]
			elif positiveValue > self.curMax[i]:
				positiveValue = self.curMax[i]
			position.append(positiveValue)
		return position
